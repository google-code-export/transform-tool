package com.vstyran.transform.controls
{
	...
	public class Control extends SkinnableComponent implements IAnchor
	{
		...
		protected function downHandler(event:MouseEvent):void
		{
			...
			tool.startTransformation(operation.type, operation);
		...
		{
			if(operation)
				tool.doTransformation(operation.doOperation( MathUtil.roundPoint(matrix.transformPoint(new Point(event.stageX, event.stageY)))), operation.type, operation);
		...
		{
			if(operation)
				tool.endTransformation(operation.endOperation( MathUtil.roundPoint(matrix.transformPoint(new Point(event.stageX, event.stageY)))), operation.type, operation);
