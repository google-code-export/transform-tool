package com.vstyran.transform.view
{
	...
	import com.vstyran.transform.managers.TransformToolGroup;
	...
	public class TransformTool extends SkinnableComponent
	{
		 ...
		 * @private 
		 */		
		private var _transformToolGroup:TransformToolGroup;

		/**
		 * Transform tool group, used for multiselection (MS PowerPoint model).
		 */		
		public function get transformToolGroup():TransformToolGroup
		{
			return _transformToolGroup;
		}

		/**
		 * @private 
		 */
		public function set transformToolGroup(value:TransformToolGroup):void
		{
			if(_transformToolGroup)
				_transformToolGroup.removeInstance(this);
			
			_transformToolGroup = value;
			
			if(_transformToolGroup)
				_transformToolGroup.addInstance(this);
		}
		 ...
		 */		
		tt_internal function startTransformation(type:String, operation:IOperation = null, isUserAction:Boolean = true):void
		{
			...
			dispatchEvent(new TransformEvent(TransformEvent.TRANSFORMATION_START, type, connector.getData(), null, isUserAction));
			
			if(transformToolGroup && operation && isUserAction)
				transformToolGroup.startTransformation(this, operation);
		}
		
		/**
		 * Transformation is in progress (mouse move on control) 
		 */	
		tt_internal function doTransformation(data:DisplayData, type:String, operation:IOperation = null, isUserAction:Boolean = true):void
		{
			DataUtil.applyData(this, data);
			
			var targetData:DisplayData = connector.transfrom(data);
			
			dispatchEvent(new TransformEvent(TransformEvent.TRANSFORMATION, type, data, targetData, isUserAction));
			
			if(transformToolGroup && operation && isUserAction)
				transformToolGroup.doTransformation(this, operation);
		 ...
		 */		
		tt_internal function endTransformation(data:DisplayData, type:String, operation:IOperation = null, isUserAction:Boolean = true):void
		{
			_transforming = false;
			...
			{			
				...
				dispatchEvent(new TransformEvent(TransformEvent.TRANSFORMATION_COMPLETE, type, data, targetData, isUserAction));
				
				if(transformToolGroup && operation && isUserAction)
					transformToolGroup.endTransformation(this, operation);
			}
			isTransformed = false;
		}
		...
		protected function moveShortcutHandler(event:KeyboardEvent):void
		{
			...
			isTransformed = true;
			endTransformation(data, TransformationType.MOVE_SHORTCUT, (moveControl ? moveControl.operation : null));
		}
